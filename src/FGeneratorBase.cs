        ...
        private bool IsSyntaxTargetForGeneration(SyntaxNode node)
        {
            ...
            bool isSupported = node switch
            {
                VariableDeclaratorSyntax vds => vds.Parent is VariableDeclarationSyntax vdsParent && vdsParent.Parent is MemberDeclarationSyntax mds && (
                    (mds is FieldDeclarationSyntax && (targets & DiscoveryTargets.Field) != 0) ||
                    (mds is EventFieldDeclarationSyntax && (targets & DiscoveryTargets.Event) != 0)
                ),
                BaseTypeDeclarationSyntax or DelegateDeclarationSyntax => (targets & DiscoveryTargets.Type) != 0,
                EnumMemberDeclarationSyntax => (targets & DiscoveryTargets.Field) != 0,
                BaseMethodDeclarationSyntax => (targets & DiscoveryTargets.Method) != 0,
                EventDeclarationSyntax => (targets & DiscoveryTargets.Event) != 0,
                PropertyDeclarationSyntax or IndexerDeclarationSyntax => (targets & DiscoveryTargets.Property) != 0,
