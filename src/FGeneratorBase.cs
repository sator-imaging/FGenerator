...
namespace FGenerator
{
    ...
    public abstract class FGeneratorBase : IIncrementalGenerator
    {
        ...
        /// <summary>
        /// Indicates which symbols to discover as targets.
        /// Defaults to <see cref="DiscoveryTargets.Type"/>.
        /// </summary>
        protected virtual DiscoveryTargets DiscoveryTargets => DiscoveryTargets.Type;
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            {
                _targetAttributeBaseName = TargetAttributeName.EndsWith("Attribute", StringComparison.Ordinal)
...
";
        ...
        private bool IsSyntaxTargetForGeneration(SyntaxNode node)
        {
            ...
            bool isSupported = node switch
            {
                VariableDeclaratorSyntax vds => vds.Parent is VariableDeclarationSyntax vdsParent && vdsParent.Parent is MemberDeclarationSyntax mds && (
                    (mds is FieldDeclarationSyntax && targets.HasFlag(DiscoveryTargets.Field)) ||
                    (mds is EventFieldDeclarationSyntax && targets.HasFlag(DiscoveryTargets.Event))
                ),
                BaseTypeDeclarationSyntax or DelegateDeclarationSyntax => targets.HasFlag(DiscoveryTargets.Type),
                EnumMemberDeclarationSyntax => targets.HasFlag(DiscoveryTargets.Field),
                BaseMethodDeclarationSyntax => targets.HasFlag(DiscoveryTargets.Method),
                EventDeclarationSyntax => targets.HasFlag(DiscoveryTargets.Event),
                PropertyDeclarationSyntax or IndexerDeclarationSyntax => targets.HasFlag(DiscoveryTargets.Property),

                ParameterSyntax or TypeParameterSyntax or CompilationUnitSyntax => true,
                _ => false
            };
            ...
            {
                VariableDeclaratorSyntax vds => ((MemberDeclarationSyntax)vds.Parent!.Parent!).AttributeLists,
        ...
        private Target? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
        {
            ...
            {
                VariableDeclaratorSyntax vds => context.SemanticModel.GetDeclaredSymbol(vds),
