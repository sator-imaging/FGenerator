...
public abstract class FGeneratorBase : IIncrementalGenerator
{
    ...
    ///     Optional (default false); setting this to false when attributes are used can improve performance by avoiding the
    ///     combine step.
    /// </summary>
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        if (!string.IsNullOrWhiteSpace(postInit))
            ...
            {
                ctx.AddSource(
                    $"- {nameof(FGenerator)} - {GetType().Name}.g.cs",
        ...
        context.RegisterSourceOutput(provider, (spc, target) =>
        {
            ...
            var codeGen = Generate(target, out var diagnosticResult);
            ...
            if (codeGen.HasValue) spc.AddSource(codeGen.Value.HintName, GenerateSourceText(codeGen.Value.Source));
        });
    }
    ...
    private SourceText GenerateSourceText(string source)
    {
        if (!source.Contains("<auto-generated", StringComparison.OrdinalIgnoreCase))
        {
            _generatedCodeHeader ??=
                $@"// <auto-generated>{GetType().FullName}</auto-generated>
...
";
    ...
    private bool IsSyntaxTargetForGeneration(SyntaxNode node)
    {
        ...
        {
            BaseFieldDeclarationSyntax => default, // handled by VariableDeclaratorSyntax
            MemberDeclarationSyntax m => m.AttributeLists, // types, methods, properties, fields, events, etc.
            VariableDeclaratorSyntax v when v.Parent is VariableDeclarationSyntax vd &&
                                            vd.Parent is BaseFieldDeclarationSyntax bf => bf.AttributeLists,
            CompilationUnitSyntax cu => cu.AttributeLists, // assembly/module-level attributes
            _ => default
        };

        if (attributeLists.Count == 0) return false;
    ...
    private Target? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        {
            VariableDeclaratorSyntax v => context.SemanticModel.GetDeclaredSymbol(v),
        ...
        if (symbol == null || symbol.IsImplicitlyDeclared) return null;
        ...
        if (matchingAttributes.IsEmpty) return null;
    ...
    private static IEnumerable<Target> EnumerateTypes(Compilation compilation, INamespaceSymbol namespaceSymbol)
    {
        foreach (var member in namespaceSymbol.GetMembers())
        {
            ...
            {
                foreach (var t in EnumerateTypes(compilation, childNamespace)) yield return t;
            }
            else if (member is INamedTypeSymbol typeSymbol)
            {
                ...
                foreach (var nested in EnumerateNestedTypes(compilation, typeSymbol)) yield return nested;
            }
        }
    }
    ...
    private static IEnumerable<Target> EnumerateNestedTypes(Compilation compilation, INamedTypeSymbol typeSymbol)
    {
        ...
        {
            if (nestedType.IsImplicitlyDeclared) continue;

            yield return CreateTargetFromType(compilation, nestedType);

            foreach (var deep in EnumerateNestedTypes(compilation, nestedType)) yield return deep;
        }
    }
