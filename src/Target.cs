...
public sealed class Target : IEquatable<Target>
{
    ...
    public ImmutableArray<ITypeParameterSymbol> GenericTypeParameters
    {
        get
        {
            if (IsGeneric)
            {
                if (RawSymbol is INamedTypeSymbol nts) return nts.TypeParameters;

                if (RawSymbol is IMethodSymbol ms) return ms.TypeParameters;
            }
        ...
        get
        {
            if (RawSymbol is ITypeSymbol type)
            {
                ...
                static IEnumerable<ITypeSymbol> enumerateDepthFirst(ITypeSymbol type)
                {
                    foreach (var member in type.GetMembers())
                    {
                        if (member.IsImplicitlyDeclared || member is not ITypeSymbol nestedType) continue;

                        yield return nestedType;

                        foreach (var deep in enumerateDepthFirst(nestedType)) yield return deep;
                    }
                }
            }
    ...
    /// </summary>
    public ImmutableStack<INamedTypeSymbol> ContainingTypes => Utils.GetContainingTypes(RawSymbol, out _);

    /// <inheritdoc />
    public bool Equals(Target? other)
    {
        return SymbolEqualityComparer.Default.Equals(RawSymbol, other?.RawSymbol);
    }
    ...
    {
        return SymbolEqualityComparer.Default.GetHashCode(RawSymbol);
    }
