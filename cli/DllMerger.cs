...
public static class DllMerger
{
    public static FileInfo? Merge(FileInfo input, DirectoryInfo outputDir)
    {
        ...
        EnsureMissingReferencesStubbed(new FileInfo(primaryDll), outputDir, tempDllFileList);
        ...
        if (otherDlls.Length == 0)
        {
            ...
            return new FileInfo(primaryDll);
        }
    ...
    private static void EnsureMissingReferencesStubbed(FileInfo primaryDll, DirectoryInfo outputDir,
        string[] existingDlls)
    {
        ...
        {
            if (existingNames.Contains(referenceName)) continue;
    ...
    private static IEnumerable<string> GetReferencedAssemblyNames(FileInfo assemblyFile)
    {
        try
        {
            ...
            if (!peReader.HasMetadata) return Array.Empty<string>();
...
");
        ...
        var exitCode = Utils.ExecuteProcess("dotnet", args, true);
        if (exitCode != 0)
        {
