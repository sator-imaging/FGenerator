...
{
    private const string AnsiColorRed = "\u001b[97;41m";
    private const string AnsiColorYellow = "\u001b[97;43m";
    ...
    public static int ExecuteProcess(string exe, string arguments, bool disableSucceededBuildStdout = false)
    {
        ...
        if (!string.IsNullOrWhiteSpace(error_text)) Console.Error.WriteLine(error_text);
    ...
    public static void GenerateUnityMeta(FileInfo dllFile, bool force)
    {
        ...
        if (!PromptOverwrite(metaPath, force)) return;
...
";
    ...
    private static string ColorizeInternal(string text)
    {
        return LogRegex.WarningOrError().Replace(text, match =>
        {
            var color = match.Value.StartsWith("err", StringComparison.OrdinalIgnoreCase)
                ? AnsiColorRed
                : AnsiColorYellow;
